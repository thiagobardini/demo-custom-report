...
export async function checkFullPageScreenshot({
  page,
  screenshotName,
  fullPage = false,
}: ScreenshotTestParams): Promise<void> {
...
export async function checkFullPageScreenshotWithMask({
  page,
  screenshotName,
  fullPage = false,
  maskTestIds = [],
  ...
  const mask = maskTestIds.map((testId) =>
...
export async function checkElementScreenshotWithMask({
  page,
  testId,
  screenshotName,
  maskTestIds = [],
...
export async function checkScreenshotPixelRatio({
  page,
  testId,
  screenshotName,
  maxDiffPixelRatioNum,
}: ScreenshotTestParams): Promise<void> {
  const locator = page.locator(`[data-testid="${testId}"]`);
  await locator.waitFor({ state: "visible" });
  await page.waitForTimeout(1000);
