...
const Report = () => {
  const { data, error, isLoading } = useSWR<ApiReportResponse>(
    '/api/reports',
    fetcher
  )
  const { data: indexData, isLoading: isIndexLoading } = useSWR<ApiResponse>(
    '/api/index',
    fetcher
  )
  ...
  const reportData = useMemo(() => {
    return data?.reports.filter(
      (report) =>
        report?.testDescribeId === `${reportNameParam}-${reportIdParam}`
    )
  ...
  if (reportIdParam) {
    const findReportId = indexData?.indexData?.some(
      (report) => report.reportId === reportIdParam
    )
  ...
  return (
    <>
      ...
      {/* {selectReportContext === 0 && <TicketsCovered />} */}
              ...
              <Table striped highlightOnHover withTableBorder>
                ...
                <Table.Tbody>
                  {filteredTests?.map((item) => (
                    <Table.Tr
                      key={item.specId}
                      id={`testId-${item.specId}`}
                      style={{ scrollMarginTop: '110px' }}
                    >
                      <Table.Td>{item.specId}</Table.Td>
                      <Table.Td>
                        <Badge
                          color={item.specStatus === 'Pass' ? 'green' : 'red'}
                        >
                          {item.specStatus}
                        </Badge>
                      </Table.Td>
                      ...
                      <Table.Td>
                        <ImageAccordionModal
                          imageUrl={item.specScreenshotPath}
                          message={item.specMessage}
                          title={item.specUrl}
                        />
                      </Table.Td>
